...
/// Highly customizable, feature-packed Flutter calendar_view with gestures, animations and multiple formats.
class TableCalendar<T> extends StatefulWidget {
  ...
  /// Determines the visibility of calendar_view header.
  ...
  /// will jump to the calendar_view page of the tapped month.
  final bool pageJumpingEnabled;

  /// When set to true, updating the `focusedDay` will display a scrolling animation
  /// if the currently visible calendar_view page is changed.
  ...
  /// Specifies the duration of scrolling animation that takes place whenever the visible calendar_view page is changed.
  final Duration pageAnimationDuration;

  /// Specifies the curve of scrolling animation that takes place whenever the visible calendar_view page is changed.
  ...
  /// If `AvailableGestures.none` is used, the calendar_view will only be interactive via buttons.
  ...
  /// Called whenever currently visible calendar_view page is changed.
  final void Function(DateTime focusedDay)? onPageChanged;

  /// Called whenever `calendarFormat` is changed.
  final void Function(CalendarFormat format)? onFormatChanged;

  /// Called when the calendar_view is created. Exposes its PageController.
...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
        ...
        if (!isDisabled) {
          ...
          if (events.isNotEmpty && markerWidget == null) {
            ...
            markerWidget = Positioned(
                top: 4,
                right: 4,
                child: _buildSingleMarker(day, events.length, markerSize));
          }
  ...
  Widget _buildSingleMarker(DateTime day, int numEvents, double markerSize) {
    return Container(
      width: 14,
      height: 14,
